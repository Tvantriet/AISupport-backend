	...
	return {
		PDFExtract: vi.fn().mockImplementation(() => ({
			extractBuffer: vi.fn().mockResolvedValue({
				pages: [{ content: [{ str: "Mock" }, { str: "parsed" }, { str: "PDF" }, { str: "content" }] }],
			}),
		})),
	};
});
...
class TestableDocumentProcessingService extends DocumentProcessingService {
	...
	public async processDocumentsToVectorPoints(
		documents: any[],
...
describe("DocumentProcessingService", () => {
	...
	describe("extractDocumentContent", () => {
		...
		it("should throw an error for invalid document format", async () => {
			await expect(service.extractDocumentContent({ invalid: "format" })).rejects.toThrow(
	...
	describe("convertFileToText", () => {
		...
		it("should throw an error for unsupported file type", async () => {
			await expect(service.convertFileToText("test.unknown")).rejects.toThrow("Unsupported file type");
		});
	});
	...
	describe("processDocumentsToVectorPoints", () => {
		it("should process multiple documents into vector points", async () => {
			...
			vi.spyOn(service, "extractDocumentContent")
